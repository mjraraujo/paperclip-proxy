...
/**
 * Detect if OPENAI_BASE_URL is pointing to a local Codex Gateway proxy.
 * The gateway listens on 127.0.0.1:18923 by default.
 */
function isProxyAuth(env: Record<string, string>): boolean {
  ...
  try {
    ...
    return (
      ...
      (parsed.port === "18923" || env.PAPERCLIP_PROXY_MODE === "1")
    );
  } catch {
    return false;
  }
...
function resolveCodexBillingType(env: Record<string, string>): "api" | "subscription" | "proxy_subscription" {
  // Codex routed through the local Codex Gateway proxy → subscription via ChatGPT device-code login.
  if (isProxyAuth(env)) return "proxy_subscription";
  // Codex uses API-key auth when OPENAI_API_KEY is present; otherwise rely on local login/session auth.
  return hasNonEmptyEnvValue(env, "OPENAI_API_KEY") ? "api" : "subscription";
}

function resolveCodexBiller(env: Record<string, string>, billingType: "api" | "subscription" | "proxy_subscription"): string {
  if (billingType === "proxy_subscription") return "chatgpt";
