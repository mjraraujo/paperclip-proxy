...
- Set env.OPENAI_BASE_URL to "http://127.0.0.1:18923" (no OPENAI_API_KEY needed; the gateway provides the token from device-code login)
- The Codex Gateway (proxy/bin/gateway.js) must be running with a valid token from interactive OpenAI device-code login
