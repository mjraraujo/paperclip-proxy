...
/**
 * Detect if ANTHROPIC_BASE_URL is pointing to a local Codex Gateway proxy.
...
function resolveClaudeBillingType(env: Record<string, string>): "api" | "subscription" | "metered_api" | "proxy_subscription" {
  if (isProxyAuth(env)) return "proxy_subscription";
  if (isBedrockAuth(env)) return "metered_api";
  return hasNonEmptyEnvValue(env, "ANTHROPIC_API_KEY") ? "api" : "subscription";
}

function resolveClaudeBiller(env: Record<string, string>): string {
  if (isProxyAuth(env)) return "chatgpt";
  if (isBedrockAuth(env)) return "aws_bedrock";
  return "anthropic";
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const toAdapterResult = (
    ...
    return {
      ...
      provider: isProxyAuth(effectiveEnv) ? "openai" : "anthropic",
      biller: resolveClaudeBiller(effectiveEnv),
